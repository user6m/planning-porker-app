...
見積もりの時間を区切りたいときは、「公開する」の行にある「⏱ タイマー」から長さ（30秒〜5分）を選ぶと、
部屋の全員の画面で同じカウントダウンが始まります。誰でも ✕ で止められ、「公開する」「リセット」でも止まります。

参加用のQRコードは「QRコードを表示」を開くと出てきます。
...
部屋は全員が抜けたあとでも同じ部屋を再利用でき、最後に使われてから30日間誰も使わなかった部屋は自動的に削除されます。
